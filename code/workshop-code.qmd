...
[deployed html url](https://sumaiyaalvi.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
drought_exp <- read_xlsx(
  path = here( #here allows you to write in file paths
    "data",
    "Valliere_etal_EcoApps_Data.xlsx"
  ) ,
  sheet = "First Harvest" #excel file had multiple sheets
)
...
drought_exp_clean <- drought_exp |> 
  ...
  clean_names() |> 
  # adding a new species_name column with the full names
  ...
  # adding a new water_treatment column for the full water treatment names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # making sure that categorical variables are factors and setting factor level order
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name, #factor with 7 levels
                                    "Lotus scoparius",
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment, #factor with 2 levels
                                       "Drought stressed",
                                       "Well watered")) |> 
...
ggplot(data = drought_exp_clean,
  ...
  #first layer: adding scatter plot points to representn individual seedlings
...
ggplot(data = drought_exp_clean,
       aes(x = species_name,
           y = total_g,
           color = species_name)) +
  #jitter to represent individual seedlings across species
  geom_jitter(height = 0,
              width = 0.2) +
  #use custom colors for points
  scale_color_manual(values = c(
    "Lotus scoparious" = lotsco_col,
  ...
  #breaks x axis labels so that they don't overlap each other
...
# model 0: null model
model0 <- lm(
  total_g ~ 1, #1 for no predictors in this model
  data = drought_exp_clean
)

# model 1: all predictors
model1 <- lm(
  total_g ~ sla + water_treatment + species_name, #saturated model w/ all predictors
  data = drought_exp_clean
)
...
AICc(
  ...
  arrange(AICc) #sort by decreasing AICcs
...
Drought stressed and Lotus scoparius (because those were the first factor levels set when the factors were made (around lines 120s))
...
This is the model prediction for the difference between Eschscholzia california mass in the well watered treatment and Lotus scoparius mass in the well watered treatment. The model predicts that this difference is 0.23 $\pm$ 0.03 g. 
...
(adjusted R^2) 72% of the variation in plant mass can be explained by this model.
...
ggplot(data = drought_exp_clean,
  ...
  facet_wrap(~ species_name,
             nrow = 1) # makes 7 different panels for each species
```
